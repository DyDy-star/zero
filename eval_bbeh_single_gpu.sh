...
# 激活 conda 环境
source /data/user5/miniconda3/etc/profile.d/conda.sh
conda activate zero
...
echo "开始 BBEH 测评"
echo "模型: $MODEL_PATH"
echo "数据集: BigBench Extra Hard"
echo "使用 GPU: $CUDA_VISIBLE_DEVICES"
echo "================================"

# 创建评估结果目录
OUTPUT_DIR="${STORAGE_PATH}/evaluation/_data_user5_R-Zero_OctoThinker-3B-Hybrid-Base"
mkdir -p ${OUTPUT_DIR}
...
output_file = '${OUTPUT_DIR}/results_bbeh.json'

tokenizer = AutoTokenizer.from_pretrained(model_path)
# ⚠️ 修改为单GPU：tensor_parallel_size=1
...
with open('${OUTPUT_DIR}/final_results_bbeh.jsonl', 'a') as f:
    json.dump({'dataset': 'bbeh', 'model': model_path, 'accuracy': round(success / (success + fail)*100, 2)}, f, indent=2)
print('Overall Accuracy:', success / (success + fail))
"

echo "================================"
echo "BBEH 测评完成！"
echo "结果保存在: ${OUTPUT_DIR}/results_bbeh.json"
echo "准确率保存在: ${OUTPUT_DIR}/final_results_bbeh.jsonl"
echo "================================"
