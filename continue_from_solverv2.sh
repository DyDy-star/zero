#!/bin/bash
#
# 从 Solver v2 开始继续训练（Solver v1 和 Questioner v2 已完成）
# 训练流程：solver_v2 -> questioner_v3 -> solver_v3
#

set -e  # 遇到错误立即退出

echo "=========================================="
echo "从 Solver v2 开始继续训练"
echo "=========================================="
echo ""
...
if [ ! -d "${STORAGE_PATH}/models/${Model_abbr}_questioner_v2/global_step_5/actor/huggingface" ]; then
    echo "❌ 错误: Questioner v2 模型不存在，请先训练 Questioner v2"
    exit 1
fi

echo "✓ Solver v1 模型已存在"
echo "✓ Questioner v2 模型已存在"
echo ""
